 ...
 *
 */
angular.module('App').controller('MassPlatdReplyEarlierCtrl', function($rootScope, $stateParams, $scope, $ajax,$uibModal ,$state) {

    $scope.currentTab = 'mass.reply.today';
    $scope.currentTab2 = 'mass.reply.earlier';
    ...
    $scope.noblack = function (mobile) {
        layer.confirm('是否去黑该手机号' + mobile +'？', {
            btn: ['确认','取消'] //按钮
        }, function(index){
            layer.close(index);

            $ajax.post('plat/black/delete', {
                mobile: mobile
            }, function () {
                layer.msg('去黑成功！', {time: 1000, icon: 1});
                $scope.load();
            });
        }, function(index){
            layer.close(index);
    ...
    $scope.sendMsg = function (mobile) {
        $uibModal.open({
            backdrop: 'static',
            templateUrl: 'sendMsgFromPlat.html',
            controller: 'sendMsgFromPlatCtrl',
            size: 'md',
            resolve:{
                data : function(){
                    return {
...
angular.module('App').controller('sendMsgFromPlatCtrl', function($rootScope, $stateParams, $scope, $ajax, $uibModalInstance, data, $uibModal) {
    $scope.params = {};
