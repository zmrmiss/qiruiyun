 ...
 *
 */
angular.module('App').controller('IntlRecordCtrl', function($rootScope, $stateParams, $scope, $ajax) {

    $scope.currentTab = 'intl.record';
    ...
    //     var url = 'intl/export/record?startDate=' + $scope.date.startDate.unix() * 1000 + '&endDate=' + $scope.date.endDate.unix() * 1000 + '&mobile=' + $scope.params.mobile + '&batchId=' + $scope.batchId + "&type=" + $scope.params.type;
    //     url = decodeURIComponent(url, true);
    //     url = encodeURI(encodeURI(url));
    //     window.location.href = url + '&content=' + $scope.params.content;
    // }
    $scope.export = function() {
        $ajax.post('intl/record/listExport', {
            startDate: $scope.date.startDate.unix() * 1000,
            endDate: $scope.date.endDate.unix() * 1000,
            mobile: $scope.params.mobile,
            content: $scope.params.content,
