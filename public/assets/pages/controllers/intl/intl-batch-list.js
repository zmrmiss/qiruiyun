 ...
 *
 */
angular.module('App').controller('MessageIntlBatchCtrl', function($rootScope, $state, $stateParams, $scope, $ajax) {

    $scope.currentTab = 'intl.batch';
    ...
    $scope.go = function(id) {
        $state.go('intl.record', {batchId: id, date: $scope.date});
    };

    $scope.export = function () {
        var url = 'intl/export/batch?startDate=' + $scope.date.startDate.unix() * 1000 + '&endDate=' + $scope.date.endDate.unix() * 1000;
